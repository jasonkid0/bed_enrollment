...
//left top right
$pdf->SetMargins(6.5, 10, 6.5);
$pdf->AddPage();

// Logo(x axis, y axis, height, width)
$pdf->Image('../../../assets/img/logo.png', 35, 5, 20, 20);
$pdf->Image('../../../assets/img/logo1.png', 169, 5, 20, 20);
// text color
...
WHERE sy.student_id = $stud_id AND sy.ay_id = '$ay_id'") or die(mysqli_error($conn));
while ($row = mysqli_fetch_array($get_stud)) {
    ...
    $cellwidth = 25;
    while ($pdf->GetStringWidth($row['section']) > $cellwidth) {
        $pdf->SetFontSize($tempFontSize -= 0.1);
    }
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->Cell(27, 4, $row['section'], 0, 0, 'L'); // for data  
    $pdf->Cell(1, 3, '', 0, 0); // space
    $pdf->SetFont('Arial', '', 8);
    ...
    while ($row3 = mysqli_fetch_array($get_enrolled_sub2)) {
        ...
        $pdf->Cell(153, 5, $row3['subject_description'], 1, 0, 'L');
        $xy += 5;
    }
    ...
    for ($x ; $x <= $addbox; $x++ ) {
        $pdf->SetXY(6.5, $laneY);
        $pdf->Cell(50, 5, '', 1, 0, 'L');
        $pdf->Cell(153, 5, '', 1, 0, 'L');
        $laneY += 5;

    }

    // box in all subject
    // $pdf->Rect(6.5, 112, 50, 60);
    // $pdf->Rect(56.5, 112, 153, 60);
